...
test_that("remstatsCpp output for directed dyadic relational events", {
    ...
    # reciprocity
    expect_equal(stats[,,15], reciprocity(el, rs, FALSE))
    # indegree_sender
    expect_equal(stats[,,16], degree(el, rs, 1, FALSE))
    # indegree_receiver
    expect_equal(stats[,,17], degree(el, rs, 2, FALSE))
    # outdegree_sender
    expect_equal(stats[,,18], degree(el, rs, 3, FALSE))
    # outdegree_receiver
    expect_equal(stats[,,19], degree(el, rs, 4, FALSE))
    # totaldegree_sender
    expect_equal(stats[,,20], degree(el, rs, 5, FALSE))
    # totaldegree_receiver
    expect_equal(stats[,,21], degree(el, rs, 6, FALSE))
    # OTP
    expect_equal(stats[,,22], triad(ac[,1], el, rs, 1, FALSE))
    # ITP
    expect_equal(stats[,,23], triad(ac[,1], el, rs, 2, FALSE))
    # OSP
    expect_equal(stats[,,24], triad(ac[,1], el, rs, 3, FALSE))
    # ISP
    expect_equal(stats[,,25], triad(ac[,1], el, rs, 4, FALSE))
...
test_that("remstatsCpp output for undirected dyadic relational events", {
    ...
    # shared_partners
    expect_equal(stats[,,11], triadU(ac[,1], el, rs, FALSE, FALSE))
    # unique_partners
    expect_equal(stats[,,12], triadU(ac[,1], el, rs, TRUE, FALSE))
})