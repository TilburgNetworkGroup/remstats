...
test_that("expected errors and warnings", {
    ...
    expect_error(
        difference(variable = "test", attr_data = info),
        "not in attr_data"
    )
    ...
    expect_error(
        difference(variable = "extraversion", attr_data = info[, -2]),
    ...
    expect_warning(
        difference(variable = "extraversion", attr_data = attr_object),
        "unexpected behavior"
    )
...
test_that("expected output from difference()", {
    ...
    expect_equal(difference(variable = "extraversion", attr_data = info), out)
})
