...
test_that("expected errors and warnings", {
  ...
  expect_error(
    minimum(variable = "test", attr_data = info),
    "not in attr_data"
  )
  ...
  expect_error(
    minimum(variable = "extraversion", attr_data = info[, -2]),
  ...
  expect_warning(
    minimum(variable = "extraversion", attr_data = attr_object),
    "unexpected behavior"
  )
  ...
  mod <- ~ minimum(variable = "extraversion", attr_data = attr_object)
  expect_warning(
...
test_that("expected output from minimum()", {
  ...
  expect_equal(minimum(variable = "extraversion", attr_data = info), out)
})
