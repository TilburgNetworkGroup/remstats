...
test_that("expected errors and warnings", {
  ...
  expect_error(
    remstats(reh = reh_actor, sender_effects = mod, attributes = attr),
    "time variable is missing"
  )
  ...
  expect_error(
    remstats(reh = reh_undirected, tie_effects = mod, attributes = info),
...
test_that("expected statistic actor-oriented model", {
  ...
  aomres <- remstats(
    reh = reh_actor, sender_effects = mod, attributes = info
  )
  ...
  # Repeat for standardized effects
  mod <- ~ send("extraversion", scaling = "std")
  aomres <- remstats(reh = reh_actor, sender_effects = mod, attributes = info)
