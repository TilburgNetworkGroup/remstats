...
test_that("adjacency matrix", {
	...
	out <- tomstats(effects = ~ inertia(), reh = reh, get_adjmat = TRUE)
	adjmat <- attr(out, "adjmat")
	expect_equal(rowSums(adjmat), 0:(nrow(history)-1))
	
	# Start and stop
	out <- tomstats(effects = ~ inertia(), reh = reh, 
		start = 5, stop = 10, get_adjmat = TRUE)
	adjmat <- attr(out, "adjmat")
	expect_equal(rowSums(adjmat), 4:9)
	
	# Windowed memory
	out <- tomstats(effects = ~ inertia(), reh = reh, 
		memory = "window", memory_value = 400, get_adjmat = TRUE)
	adjmat <- attr(out, "adjmat")
	...
	out <- tomstats(effects = ~ inertia(), reh = reh, 
		memory = "decay", memory_value = 400, get_adjmat = TRUE)
	adjmat <- attr(out, "adjmat")
	expect_equal(rowSums(adjmat),
		apply(history, 1, function(x) {
