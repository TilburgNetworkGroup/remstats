...
Specifies the statistic for a "receive" effect in the tie-oriented model
or the receiver choice step of the actor-oriented model. A "receive"
effect refers to an exogenous actor attribute that affects actor *i*'s
rate of receiving events.
...
attribute for actor *i* at time *t* for all dyads in the riskset that
have actor *i* as receiver. Note that a "receive" effect is only defined
for directed relational events.
