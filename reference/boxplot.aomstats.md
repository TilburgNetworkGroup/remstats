...
- effect:
  ...
  A string specifying the name of the effect in 'x' or an integer
  indicating the index of the effect to be plotted.

- model:

  A string indicating whether the effect is in the 'sender' model or the
  'receiver' model.
...
- subset:
  ...
  An optional vector specifying a subset of timepoints or actors to be
...
index within the 'aomstats' object. The 'model' parameter indicates
whether the respective effect is in the 'sender' model or the 'receiver'
model. The 'by' parameter determines whether the boxplots are created
across different 'timepoints' or 'actors'. At the moment, by 'actors' is
only supported for the sender model. Additionally, an optional 'subset'
...
actors are plotted. The 'outliers' argument, when set to TRUE, includes
the representation of outliers in the boxplots. If set to FALSE,
outliers are omitted from the visualization.
