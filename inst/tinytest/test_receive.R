...
expect_error(receive(), pattern = "missing")
# Expect error for wrong "variable"
expect_error(receive(variable = "extroversion", attributes = info),
  pattern = "not in attributes"
)
# Expect error for missing time variable
expect_error(receive(variable = "extraversion", attributes = info[, -2]),
  pattern = "time variable is missing"
)
# Expect warning for missing values
info$extraversion[1] <- NA
expect_warning(receive(variable = "extraversion", attributes = info),
...
# Expect error for wrong "variable"
mod <- ~ receive(variable = "extroversion")
...
mod <- ~ receive(variable = "extraversion", attributes = info)
...
mod <- ~ receive(variable = "extraversion")
expect_error(
  remstats(edgelist = history, receiver_effects = mod, attributes = info[, -2]),
  pattern = "time variable is missing"
)
# Expect error for receiver effects
expect_error(
  remstats(edgelist = history, sender_effects = mod, attributes = info),
  pattern = "not defined for the sender activity model"
)
# Expect warning for missing values
info$extraversion[1] <- NA
expect_warning(
...
mod <- ~ receive(variable = "extraversion", attributes = info)
expect_error(remstats(edgelist = history, receiver_effects = mod),
  pattern = "Missing actors"
)
# Re-load info
data(info)

# Expected output from receive() [effects.R] ---------------------------------
# Expected standard output
out <- list(effect = "receive", variable = "extraversion", x = NULL, scaling = 1)
...
stat1 <- first_info$extraversion[match(tomres$riskset$reicever, first_info$id)]
...
stat2 <- second_info$extraversion[match(tomres$riskset$reicever, second_info$id)]
...
stat3 <- third_info$extraversion[match(tomres$riskset$reicever, third_info$id)]
...
mod <- ~ receive("extraversion")
aomres <- remstats(edgelist = history, reicever_effects = mod, attributes = info)

# Expected name of the statistic
expect_equal(
  dimnames(aomres$statistics$reicever_stats)[[3]][2],
...
expect_true(all(sapply(1:40, function(x) {
  all(stat1 == aomres$statistics$reicever_stats[x, , 2])
})))
...
expect_true(all(sapply(41:71, function(x) {
  all(stat2 == aomres$statistics$reicever_stats[x, , 2])
})))
...
expect_true(all(sapply(72:115, function(x) {
  all(stat3 == aomres$statistics$reicever_stats[x, , 2])
})))
...
stat1 <- as.numeric(scale(stat1))
expect_true(all(sapply(1:40, function(x) {
	all.equal(stat1, aomres$statistics$reicever_stats[x,,2], 
...
stat2 <- as.numeric(scale(stat2))
expect_true(all(sapply(41:71, function(x) {
	all.equal(stat2, aomres$statistics$reicever_stats[x,,2], 
  check.attributes = FALSE)
})))
...
stat3 <- as.numeric(scale(stat3))
expect_true(all(sapply(72:115, function(x) {
  all.equal(stat3, aomres$statistics$reicever_stats[x,,2], 
  check.attributes = FALSE)
})))
